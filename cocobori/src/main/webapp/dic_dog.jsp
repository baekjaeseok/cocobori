...
<body class="is-preload" style="font-family: 'omyu_pretty'">
    ...
    <%
		...
		List<CocoPetDTO> petList = dao.selAllDog();
	%>
	<!-- Header -->
	<div id="header">
	...
	<div id="main">
		...
		<section id="portfolio" class="two" style="padding-top: 20px; padding-bottom: 20px;">
				<img class="icon_pet" alt="" src="./images/icon_dog.gif"><span class="sub_title">강아지 백과사전</span>
			<div class="container-a" style="width: 70%; font-size: 18px;">

				<form action="SelectDog" method="post" enctype="multipart/form-data">
				...
				%>
				<!-- 동영상 올리기 -->
				<video style="margin-top: 10px; border-radius: 15px;" src="./images/buldog.mp4" autoplay="autoplay" muted="muted" loop="loop" width="30%" height="50%">
				</video>
				<%
					}
				%>
				...
				<table class="table-a" style="border: 1px solid black;">

					<tr>
						<td class="pet-score">적응력 <%=pet.getPet_adaptno()%></td>
					</tr>
					<tr>
						<td class="pet-state" colspan="3"><%=pet.getPet_adapt()%></td>
					</tr>
					<tr>
						<td class="pet-score">실내 적합도 <%=pet.getPet_fitno()%></td>
					</tr>
					<tr>
						<td class="pet-state" colspan="3"><%=pet.getPet_fit()%></td>
					</tr>
					<tr>
					...
					<tr>
						<td class="pet-score">훈련 습득력 <%=pet.getPet_trainno()%></td>
					</tr>
					<tr>
						<td class="pet-state" colspan="3"><%=pet.getPet_train()%></td>
					</tr>
					<tr>
